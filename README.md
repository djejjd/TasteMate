...
TasteMate 是一个面向 Hermes 的外置个人偏好层。当前已完成迭代一与迭代二：先打通显式 `@taste` 的后置候选重排闭环，再把固定候选主路径替换为 Hermes 主动整理真实 candidates 后调用 TasteMate 排序。

## 当前状态

```text
阶段：迭代二已完成
范围：真实 candidates 排序闭环
验证：本地测试、Hermes 插件回归边界、CLI 真实入口 A-001 / A-002 真实 candidates 路径均已验证
...
- candidates 最小协议校验：要求 `id`、`title`、`summary`、`metadata`，并对无效候选返回结构化降级结果。
- Hermes 基于用户给定候选或已有知识整理真实 candidates 后调用 TasteMate 排序。
- Hermes `tastemate-route` 插件：保留为 iteration-001 回归路径，不作为 iteration-002 真实候选主路径。
...
- 不做 UI、多用户系统、账号体系。
- 不做 observed_tool_candidates 自动抽取。
- 不把 `fixed_probe_candidates` 当成正式候选来源；它只保留为 iteration-001 回归验证路径。
...
docs/iterations/iteration-002/          迭代二文档、验证记录和收尾记录
...
- [迭代二状态](docs/iterations/iteration-002/status.md)
- [迭代二验证](docs/iterations/iteration-002/verification.md)
- [迭代二收尾](docs/iterations/iteration-002/closeout.md)
